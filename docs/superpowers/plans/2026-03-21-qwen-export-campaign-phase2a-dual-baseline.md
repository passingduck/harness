...
- if an active `baseline-qwen` task exists, closes it with `--worker-status BLOCKED`, preserves its worktree, and writes a blocked/stale packet that points to the preserved worktree and registry entry
...
```bash
scripts/harness/close-worktree.sh --task-id baseline-qwen --mode preserve --worker-status BLOCKED
bash scripts/setup/seed-runtime-state.sh
find .harness/runtime/queue -maxdepth 2 -type f | sort
sed -n '1,80p' .harness/runtime/queue/done/source-readiness-verify.md
sed -n '1,80p' .harness/runtime/queue/ready/baseline-contract-refresh.md
sed -n '1,80p' .harness/runtime/queue/ready/baseline-qwen-dual.md
sed -n '1,40p' third_party/harness-source.txt
git status --short
```
...
- stale `baseline-qwen` no longer appears as the active task and is preserved under `blocked/`
- the reseeded queue packets show concrete `owned_paths`, `required_reads`, `docs_to_update`, `verification_commands`, `review_stages`, and `dependencies`
- `third_party/harness-source.txt` records the phase-2A source commit and resync timestamp
...
scripts/harness/build-review-pack.sh --type pr --task-id baseline-contract-refresh --title "baseline-contract-refresh" --changed-path README.md --changed-path SUMMARY.md --changed-path REQUIREMENT.md --changed-path docs/experiments/eval/DIRECTORY.md --changed-path docs/experiments/harness/scorecard.md --verification-command "bash scripts/setup/seed-runtime-state.sh" --verification-command "find .harness/runtime/queue -maxdepth 2 -type f | sort" --promote-to docs/reviews/baseline-contract-refresh.md
```
...
scripts/harness/write-review-result.sh --task-id baseline-contract-refresh --stage spec_scope_review --verdict APPROVED --evidence-ref docs/reviews/baseline-contract-refresh.md --next-action finish
scripts/harness/write-review-result.sh --task-id baseline-contract-refresh --stage rules_lint_review --verdict APPROVED --evidence-ref docs/reviews/baseline-contract-refresh.md --next-action finish
scripts/harness/write-review-result.sh --task-id baseline-contract-refresh --stage adversarial_regression_review --verdict APPROVED --evidence-ref docs/reviews/baseline-contract-refresh.md --next-action finish
scripts/harness/write-review-result.sh --task-id baseline-contract-refresh --stage human_review_pack_review --verdict APPROVED --evidence-ref docs/reviews/baseline-contract-refresh.md --next-action finish
...
## Task 3: Claim `baseline-qwen-dual`, Salvage the Stale Worktree, and Implement the Text Lane
...
- [ ] **Step 2: Salvage only the reusable pieces from the stale `baseline-qwen` worktree**

Inspect the preserved stale worktree and runtime metadata:

```bash
sed -n '1,120p' .harness/runtime/queue/blocked/baseline-qwen-stale.md
sed -n '1,120p' .harness/runtime/worktree-registry/baseline-qwen.md
git -C .worktrees/baseline-qwen status --short
```

Then decide explicitly which pieces to port forward and which to reject. Keep durable note(s) in the review narrative about:

- reusable helper/test logic that is carried forward
- stale `4bit`/offload assumptions that are intentionally dropped

- [ ] **Step 3: Write failing config and loader tests inside the `baseline-qwen-dual` worktree**
...
- [ ] **Step 4: Run the targeted tests to watch them fail**
...
- [ ] **Step 5: Implement the minimal config and loader surface**
...
- [ ] **Step 6: Add failing text-runner tests**
...
- [ ] **Step 7: Run the new text-runner tests to verify the red phase**
...
- [ ] **Step 8: Implement the minimal text runner and wrapper**
...
- [ ] **Step 9: Run the baseline unit suite**
...
- [ ] **Step 10: Execute the real text baseline**
...
## Task 4: Add the Fixed-Sample COCO VL Smoke Lane Inside `baseline-qwen-dual`
...
- writes a stable artifact payload per sample, including sample identifier and a short human note field
...
- update `docs/experiments/eval/baseline-coco-vl-smoke.md` with the fixed sample-selection rule, prompt/image/caption triplets, sample identifiers, and short human notes
...
- `docs/experiments/eval/baseline-coco-vl-smoke.md` records the fixed sample-selection rule plus prompt/image/caption triplets, sample identifiers, and short human notes
...
## Task 5: Finish `baseline-qwen-dual` Through the Harness
...
- [ ] **Step 1: Run both real baseline wrappers inside the active `baseline-qwen-dual` worktree**
...
- [ ] **Step 2: Update the harness scorecard from the task outcome**
...
- [ ] **Step 3: Build review artifacts and finish the worktree**
...
scripts/harness/build-review-pack.sh --type pr --task-id baseline-qwen-dual --title "baseline-qwen-dual" --changed-path src/baseline/config.py --changed-path src/baseline/loader.py --changed-path src/baseline/run_text_baseline.py --changed-path src/baseline/run_vl_smoke.py --changed-path src/baseline/coco_smoke.py --changed-path docs/experiments/eval/baseline-wikitext2.md --changed-path docs/experiments/eval/baseline-coco-vl-smoke.md --changed-path docs/experiments/harness/scorecard.md --verification-command "python3 -m unittest discover -s tests/baseline -p 'test_*.py' -v" --verification-command "bash scripts/eval/run-baseline-wikitext2.sh" --verification-command "bash scripts/eval/run-vl-coco-smoke.sh" --promote-to docs/reviews/baseline-qwen-dual.md
scripts/harness/run-qa.sh --stage rules-lint
scripts/harness/run-qa.sh --stage adversarial-regression
scripts/harness/write-review-result.sh --task-id baseline-qwen-dual --stage spec_scope_review --verdict APPROVED --evidence-ref docs/reviews/baseline-qwen-dual.md --next-action finish
scripts/harness/write-review-result.sh --task-id baseline-qwen-dual --stage rules_lint_review --verdict APPROVED --evidence-ref docs/reviews/baseline-qwen-dual.md --next-action finish
scripts/harness/write-review-result.sh --task-id baseline-qwen-dual --stage adversarial_regression_review --verdict APPROVED --evidence-ref docs/reviews/baseline-qwen-dual.md --next-action finish
scripts/harness/write-review-result.sh --task-id baseline-qwen-dual --stage human_review_pack_review --verdict APPROVED --evidence-ref docs/reviews/baseline-qwen-dual.md --next-action finish
